...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const records = found.map(unwrapRecord).filter((item) => item && pick(item, 'ID', 'id')).slice(0, MAX_RESULTS);
    ...
    for (const record of records) {
      ...
      const platform = String(readCustomField(item, tcField) || tc).trim();

      const [client, responsible, stage, afacturar] = await Promise.all([
        resolveDealClient(webhookUrl, item, id),
        resolveUser(webhookUrl, assignedById),
        resolveStage(webhookUrl, categoryId, stageId),
        lookupAfacturar({ platform }).catch(() => null)
      ]);
...
async function findDeals(webhookUrl, tcField, tc) {
  ...
  try {
    const deals = await callBitrix(webhookUrl, 'crm.deal.list', {
      ...
      'order[DATE_MODIFY]': 'DESC', start: 0
    });
  ...
  try {
    const result = await callBitrix(webhookUrl, 'crm.item.list', {
      entityTypeId: 2, useOriginalUfNames: 'Y', [`filter[${tcField}]`]: tc,
      'select[]': ['id','title','stageId','categoryId','companyId','contactId','assignedById','dateCreate','dateModify',tcField], start: 0
    });
  ...
  if (!contactId && dealId) {
    try {
      ...
      if (Array.isArray(contacts) && contacts.length) {
        ...
        contactId = normalizeId(pick(primary, 'CONTACT_ID', 'contactId', 'CONTACTID'));
      }
    } catch {}
  }
  if (companyId) {
    ...
    } catch { return `Empresa ${companyId}`; }
  }
  if (contactId) {
    try {
      ...
      const name = [pick(contact, 'NAME', 'name'), pick(contact, 'LAST_NAME', 'lastName')].filter(Boolean).join(' ').trim();
      return name.slice(0, 300) || `Contacto ${contactId}`;
    } catch { return `Contacto ${contactId}`; }
  }
...
async function resolveStage(webhookUrl, categoryIdValue, stageIdValue) {
  ...
  try {
    ...
    const match = (Array.isArray(stages) ? stages : []).find((item) => String(pick(item, 'STATUS_ID', 'statusId') || '') === stageId);
    return String(pick(match, 'NAME', 'name') || stageId);
  } catch { return stageId; }
}

function unwrapRecord(value) { if (!value || typeof value !== 'object') return null; return value.item || value.fields || value.deal || value; }
function pick(object, ...keys) { for (const key of keys) if (object?.[key] !== undefined && object?.[key] !== null) return object[key]; const entries=Object.entries(object||{}); for(const key of keys){const match=entries.find(([name])=>name.toLowerCase()===String(key).toLowerCase()); if(match)return match[1];} return null; }
function readCustomField(object, apiName) { const camelName=String(apiName).toLowerCase().replace(/_([a-z0-9])/g,(_,c)=>c.toUpperCase()); return pick(object,apiName,apiName.toLowerCase(),camelName); }
function normalizeId(value){const id=String(value||'').trim();return id&&id!=='0'?id:'';}
