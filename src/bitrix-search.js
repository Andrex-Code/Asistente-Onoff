(() => {
  ...
  function renderDeals(deals, tc) {
    ...
    resultBox.innerHTML = '';
    deals.forEach(deal => resultBox.appendChild(card(
      `TC${deal.tc || tc}`,
      deal.title,
      [['Estado',deal.stage],['Cliente',deal.client],['Responsable',deal.responsible],['Última actualización',formatDate(deal.updatedAt)],['ID',deal.id]],
      deal.url,
      'Abrir negociación',
      deal.afacturar
    )));
  ...
  function renderCompanies(companies, masked) {
    ...
    companies.forEach(company => {
      const node = card('Cliente', company.name, [['Identificación',masked],['Responsable',company.responsible],['Última actualización',formatDate(company.updatedAt)],['ID',company.id]], company.url, 'Abrir cliente', company.afacturar);
  ...
  function renderTask(task) {
    ...
    resultBox.appendChild(card(`Radicado ${task.id}`,task.title,details,task.url,'Abrir tarea',null));
  }
  ...
  function card(kicker,title,details,url,openLabel,afacturar) {
    ...
    const actions=document.createElement('div');actions.className='onoff-bitrix-actions';
    const open=document.createElement('button');open.className='is-primary';open.textContent=openLabel;open.onclick=()=>window.open(url,'_blank','noopener');actions.appendChild(open);
    if(afacturar?.url){const af=document.createElement('button');af.className='is-afacturar';af.textContent='Abrir en Afacturar';af.title=afacturar.status?`Estado Afacturar: ${afacturar.status}`:'Abrir perfil en Afacturar';af.onclick=()=>window.open(afacturar.url,'_blank','noopener');actions.appendChild(af);}
    const copy=document.createElement('button');copy.textContent='Copiar enlace';copy.onclick=async()=>{await navigator.clipboard.writeText(url);copy.textContent='Enlace copiado';setTimeout(()=>copy.textContent='Copiar enlace',1200);};actions.appendChild(copy);
  ...
  function startDrag(e){if(e.button!==0||e.target.closest('button'))return;const r=searchWindow.getBoundingClientRect();dragState={id:e.pointerId,x:e.clientX-r.left,y:e.clientY-r.top};searchWindow.setPointerCapture?.(e.pointerId);window.addEventListener('pointermove',moveDrag,true);window.addEventListener('pointerup',endDrag,true);window.addEventListener('pointercancel',endDrag,true);}
  ...
  async function endDrag(e){if(!dragState||e.pointerId!==dragState.id)return;dragState=null;window.removeEventListener('pointermove',moveDrag,true);window.removeEventListener('pointerup',endDrag,true);window.removeEventListener('pointercancel',endDrag,true);searchWindow.releasePointerCapture?.(e.pointerId);const r=searchWindow.getBoundingClientRect();await chrome.storage.local.set({[POSITION_KEY]:{left:Math.round(r.left),top:Math.round(r.top)}});}
  async function applyPosition(){const p=(await chrome.storage.local.get(POSITION_KEY))[POSITION_KEY];if(p&&Number.isFinite(Number(p.left))&&Number.isFinite(Number(p.top))){searchWindow.style.left=`${p.left}px`;searchWindow.style.top=`${p.top}px`;searchWindow.style.right='auto';searchWindow.style.bottom='auto';keepInside();}}
  function keepInside(){if(!searchWindow||searchWindow.hidden)return;const r=searchWindow.getBoundingClientRect();searchWindow.style.left=`${clamp(r.left,8,window.innerWidth-r.width-8)}px`;searchWindow.style.top=`${clamp(r.top,8,window.innerHeight-r.height-8)}px`;searchWindow.style.right='auto';searchWindow.style.bottom='auto';}
