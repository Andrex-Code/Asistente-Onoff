...
async function readAfacturarIndex() {
  ...
  try {
    ...
    cache = normalizeStoredIndex(parsed);
...
async function importAfacturarCsv(csvText, options = {}) {
  ...
  const identificationIndex = findColumn(header, ['nitcc', 'nit', 'cc', 'identificacion', 'documento']);
  ...
  if (platformIndex < 0 || identificationIndex < 0 || urlIndex < 0) {
    throw new Error('AFACTURAR_REQUIRED_COLUMNS');
  }
  ...
  for (const row of rows.slice(1)) {
    const platform = digits(row[platformIndex]);
    const identification = digits(row[identificationIndex]);
    const url = normalizeAfacturarUrl(row[urlIndex]);
    if (!platform || !identification || !url) {
      if (row.some((value) => String(value || '').trim())) rejectedRows += 1;
      continue;
    }

    const key = `${platform}|${identification}|${url}`;
  ...
  cache = saved;
  cacheExpiresAt = Date.now() + CACHE_MS;
  return getPublicImportResult(saved);
}
...
async function lookupAfacturar(criteria = {}) {
  ...
  const platform = digits(criteria.platform);
  const identification = digits(criteria.identification);
  let candidates = [];

  if (platform) candidates = index.records.filter((record) => record.platform === platform);
  if (!candidates.length && identification) candidates = index.records.filter((record) => record.identification === identification);
  if (!candidates.length) return null;

  const active = candidates.find((record) => normalizeHeader(record.status) === 'activo') || candidates[0];
  return {
    url: active.url,
...
function getPublicImportResult(index) {
  return {
    ready: true,
    count: index.records.length,
...
function normalizeStoredIndex(value) {
  const records = Array.isArray(value?.records)
    ? value.records.map((record) => ({
        platform: digits(record?.platform),
        identification: digits(record?.identification),
        url: normalizeAfacturarUrl(record?.url),
        status: String(record?.status || '').slice(0, 40),
        name: String(record?.name || '').slice(0, 180)
      })).filter((record) => record.platform && record.identification && record.url)
    : [];
  return {
    version: 1,
...
function normalizeAfacturarUrl(value) {
  try {
    ...
    if (!/^\/obligado\/\d+\/?$/i.test(url.pathname)) return '';
    url.search = '';
    url.hash = '';
